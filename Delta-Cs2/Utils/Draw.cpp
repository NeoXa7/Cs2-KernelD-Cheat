...

{
    ...
    for (int i = 0; i < sizeof(bone.Connections) / sizeof(bone.Connections[0]); i++)
    {
        ...
        if (Project3DWorldTo2D(Position, screenPos, ViewMatrix) &&
            Project3DWorldTo2D(head, headScreenPos, ViewMatrix))
        {
            Vector2 b1, b2;
            if (Project3DWorldTo2D(VectorBone1, b1, ViewMatrix) &&
                Project3DWorldTo2D(VectorBone2, b2, ViewMatrix))
            {
