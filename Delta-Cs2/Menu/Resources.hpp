...
namespace Resources
{
	...
	{
		auto cheatIcon = LoadImageByMemory(device, SynapseX_Logo_Byte, sizeof(SynapseX_Logo_Byte), &cheat_logo);
		if (!cheatIcon)
			return false;
		
		auto cheatIcon2 = LoadImageByMemory(device, Delta_Cheat_Logo_Byte, sizeof(Delta_Cheat_Logo_Byte), &cheat_logo2);
		if (!cheatIcon2)
			return false;
