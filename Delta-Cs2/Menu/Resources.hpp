...
namespace resources
{
	...
	inline ID3D11ShaderResourceView* cheat_logo = nullptr;
	inline ID3D11ShaderResourceView* cheat_logo2 = nullptr;
	...
	{
		auto cheatIcon = LoadImageByMemory(device, synapseX_cheat_logo_bytes, sizeof(synapseX_cheat_logo_bytes), &cheat_logo);
		if (!cheatIcon)
			return false;
		
		auto cheatIcon2 = LoadImageByMemory(device, delta_cheat_logo_bytes, sizeof(delta_cheat_logo_bytes), &cheat_logo2);
		if (!cheatIcon2)
			return false;
	...
	inline void CleanupImages() {
		if (cheat_logo) { cheat_logo->Release(); cheat_logo = nullptr; }
