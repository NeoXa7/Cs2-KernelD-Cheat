...
VOID Menu::CreateEntitiesInterface()
{
    ...
    if (ImGui::BeginTable("EntityTable", 8, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
    {
        ...
        for (int i = 1; i < 64; i++)
        {
            ...
            {
            case TEAM_SPECTATOR: if (entities->Team)ImGui::TableNextColumn(); ImGui::Text("%s", "Spectator"); break;
            case TEAM_TERRORIST: if (entities->Team)ImGui::TableNextColumn(); ImGui::Text("%s", "T"); break;
            case TEAM_COUNTERTERRORIST: if (entities->Team)ImGui::TableNextColumn(); ImGui::Text("%s", "CT"); break;
            }
...
// Settings Interface Defination
VOID Menu::CreateSettingsInterface()
{
    const float xOffset = 10.0f;
    ...
    if (Flags::m_bIsVisible)
    {
        ...
        // Main Content
        {
            ...
            if (tab_selection_count == 3)
            {
                ...
                if (cplayerinfo_group_count == 7)
                    this->CreateLocalClientInfoInterface();

                if (cplayerinfo_group_count == 8)
            ...
            {
                if (custom.Group("Config", Groups::CONFIG == misc_group_count))
                    cplayerinfo_group_count = Groups::LOCALPLAYER;
                ImGui::SameLine();
                if (custom.Group("Offsets", Groups::OFFSETS == misc_group_count))
                    cplayerinfo_group_count = Groups::LOCALPLAYER;

                if (misc_group_count == Groups::CONFIG)
                {
