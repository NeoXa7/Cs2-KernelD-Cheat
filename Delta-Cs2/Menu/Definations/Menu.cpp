...
VOID Menu::CreateEntitiesInterface()
{
    ...
    if (ImGui::BeginTable("EntityTable", 8, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
    {
        ...
        for (int i = 1; i < 64; i++)
        {
            ...
            // Entity Status 
            if (cEntities->IsAlive) { ImGui::TableNextColumn(); ImGui::Text("%s", "Alive"); }
            else if (cEntities->IsDead) { ImGui::TableNextColumn(); ImGui::Text("%s", "Dead"); }
    ...
    if (Flags::m_bIsVisible)
    {
        ...
        // Main Content
        {
            ...
            if (tab_selection_count == 5)
            {
                ...
                ImGui::SameLine();
                if (custom.Group("Menu", Groups::MENU_SETTINGS == settings_group_count))
                    settings_group_count = Groups::MENU_SETTINGS;
                ...
                if (settings_group_count == Groups::MENU_SETTINGS)
                {
