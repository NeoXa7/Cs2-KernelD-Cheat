...
VOID Custom::RadioButtonGroup(int* current_index, const char* const items[], int items_size)
{
    ...
    for (int i = 0; i < items_size; ++i)
    {
        ...
        transition_progress[i] = std::clamp(transition_progress[i], 0.0f, 1.0f); // Keep within [0, 1]

        float t = 0.5f - 0.5f * std::cos(transition_progress[i] * 3.14159f); // Smooth step with easing
...
VOID Custom::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, float width)
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 5.0f); // Adjust frame rounding
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(0.1, 0.1)); // Adjust padding
    ImGui::PushStyleVar(ImGuiStyleVar_GrabRounding, 15.0f); // Adjust grab rounding

    ImGui::PushStyleColor(ImGuiCol_FrameBg, SliderBGColor.Value); // Background color of the slider track
    ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImColor(34, 34, 34, 255).Value); // Background color of the slider track when hovered
