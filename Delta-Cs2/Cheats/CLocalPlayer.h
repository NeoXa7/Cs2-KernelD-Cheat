...
namespace CLocalPlayer
{
	...
	inline VOID StandaloneRecoilControlSystem()
	{
		...
		if (cPlayer.IsFiring)
		{
			Double delta_x = (aimPunch_x - Data::Rcs::m_flOldPunchAngleX) * -1.0;
			Double delta_y = (aimPunch_y - Data::Rcs::m_flOldPunchAngleY) * -1.0;
			Int mouse_x = Int((delta_y * 2.0 / cPlayer.Sensitivity) / -0.022);
			Int mouse_y = Int((delta_x * 2.0 / cPlayer.Sensitivity) / 0.022);
