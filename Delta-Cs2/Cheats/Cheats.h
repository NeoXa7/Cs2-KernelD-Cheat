...
#include <SDK/GameObjectTypes.h>
#include <SDK/GameConstants.h>
...
public:
	...
	VOID BeginPlayerEntityLoop(uintptr_t ClientDll, uintptr_t MatchmakingDll)
	{
		...
		for (int i = 0; i < 64; i++)
		{
			Pointer ListEntry = Driver::RPM<Pointer>(game.EntityList + (8 * (i & 0x7FFF) >> 9) + 16);
			if (!ListEntry)
				continue;

			Pointer CCSPlayerController = Driver::RPM<Pointer>(ListEntry + 120 * (i & 0x1FF));
			if (!CCSPlayerController)
				continue;

			Pointer PawnHandle = Driver::RPM<Pointer>(CCSPlayerController + Offsets::m_hPlayerPawn);
			if (!PawnHandle)
				continue;

			Pointer ListEntry2 = Driver::RPM<Pointer>(game.EntityList + 0x8 * ((PawnHandle & 0x7FFF) >> 9) + 16);
			if (!ListEntry2)
				continue;

			Pointer CCSPlayerPawn = Driver::RPM<Pointer>(ListEntry2 + 120 * (PawnHandle & 0x1FF));
			if (!CCSPlayerPawn)
				continue;
	...
	VOID BeginWorldEntitiesLoop(uintptr_t ClientDll, uintptr_t MatchmakingDll)
	{
		if (Flags::VisualCheats == ENABLED)
		{
			...
			{
				Pointer ItemEntityListEntry = Driver::RPM<Pointer>(game.EntityList + 8LL * ((i & 0x7FFF) >> 9) + 16);
				if (!ItemEntityListEntry)
					continue;

				Pointer ItemEntity = Driver::RPM<Pointer>(ItemEntityListEntry + 120LL * (i & 0x1FF)); // Item Entity
				if (!ItemEntity)
					continue;

				Pointer ItemEntityNode = Driver::RPM<Pointer>(ItemEntity + Offsets::m_pGameSceneNode); // Item Entity Node
				...
				Pointer ItemInfo = Driver::RPM<Pointer>(ItemEntity + 0x10);
				Pointer ItemType = Driver::RPM<Pointer>(ItemInfo + 0x20);
				...
				if (ItemEntityOrigin.x)
				{
					...
					std::string Weapons = world.GetWeaponType(type, WeaponsType);
					std::string Projectiles = world.GetProjectileType(type, ProjectilesType);
					std::string Entity = world.GetEntityType(type, EntityType);
...
inline VOID Cheats::DrawPlayerEntityEsp()
{
		...
		Project3DWorldTo2D(HeadPosition, EntitiesHeadScreenPos, game.ViewMatrix)) // Entities W2S Head Position;
	{
		...
		if (Data::Esp::Player::m_bEnableLines == TRUE)
		{
			...
			if (game.EntitiesToLocalDist < Data::Esp::Player::m_flLineEndPointDisplayDistance)
			{
				Draw::Line
				(
					Data::Esp::Player::m_vecCurrentlinePos,
					EntitiesScreenPosition,
		...
		if (Data::Esp::Player::m_bEnableNames == TRUE)
		{
			...
			float distanceFactor = 1.0f / game.EntitiesToLocalDist; // Scale the font inversely with distance
		...
		if (Data::Esp::Player::m_bEnableBones == TRUE)
		{
			...
			if (Data::Esp::Player::m_bEnableBonesJoints == TRUE)
			{
				Draw::Bones(
					entities->BoneMatrix, Data::Esp::Player::m_clBoneColor,
					Data::Esp::Player::m_clBoneHeadColor, Data::Esp::Player::m_flBonesThickness,
					TRUE, Data::Esp::Player::m_bBoneHeadFilled,
			...
			else
			{
				Draw::Bones(
					entities->BoneMatrix, Data::Esp::Player::m_clBoneColor,
					Data::Esp::Player::m_clBoneHeadColor, Data::Esp::Player::m_flBonesThickness,
					false, Data::Esp::Player::m_bBoneHeadFilled
				);
			}
		}
		...
		if (Data::Esp::Player::m_bEnableDistance == TRUE)
		{
			...
			float EntitiesDistanceInMeters = game.EntitiesToLocalDist / GAME_METRIC_UNITS;
