...
{
public:
	Pointer GetMatrix(Pointer CCSPlayerPawn)
	{
		if (CCSPlayerPawn != NULL)
		{
			Pointer PlayerNode = Driver::RPM<Pointer>(CCSPlayerPawn + Offsets::m_pGameSceneNode);
			uintptr_t BoneMatrix = Driver::RPM<Pointer>(PlayerNode + Offsets::m_groundNormal);
			return BoneMatrix;
		}
		else
		{
